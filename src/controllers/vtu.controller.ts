...
export const FetchNetworkID = async (req: any, res: any, next: any) => {
    try {
        const response = await axios.get(`${process.env.VTU_URL}/get/network/`, {
            headers: {
...
export const GetDataPlans = async (req: any, res: any, next: any) => {
    try {
        const response = await axios.get(`${process.env.VTU_URL}/network`, {
            headers: {
                'Authorization': `Token ${process.env.VTU_API_KEY}`
            }
        })
        ...
        // res.status(500).send('Server Error')
            console.error('Error fetching data plans:', (ex as any).response?.data || (ex as any).message);
            res.status(500).send('Server Error');
    }
}
...
export const BuyAirTime = async (req: any, res: any, next: any) => {
    ...
    try {
        ...
        const response = await axios.post(`${process.env.VTU_URL}/topup/`, { ...payload }, {
            headers: {
                'Authorization': `Token ${process.env.VTU_API_KEY}`,
...
export const BuyData = async (req: any, res: any, next: any) => {
    ...
    try {
        ...
        const response = await axios.post(`${process.env.VTU_URL}/data/`, { ...payload }, {
            headers: {
                'Authorization': `Token ${process.env.VTU_API_KEY}`,
